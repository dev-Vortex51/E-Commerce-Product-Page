...
function Header({isClicked,setIsClicked,setIsOpen,quantity}) {
  return (
    <header className=' w-full py-3 px-5   md:max-w-screen-lg md:mx-auto  h-16 bg-white flex items-center justify-between md:border-b md:pt-12 md:pb-10 shadow-bottom-xl md:shadow-none'>
      ...
      <img src="/icon-menu.svg" alt="" className='block md:hidden' onClick={()=>setIsClicked(true)}/>
      ...
      <div className='flex space-x-4 items-center'>
      <img src="/icon-cart.svg" alt=""  className='cursor-pointer' onClick={()=> setIsOpen(prev=>!prev)}/>
        <div className='relative'>
          <img src="/image-avatar.png" alt="" className='w-6 md:w-10 cursor-pointer hover:border-2 hover:border-Orange hover:rounded-full' />
